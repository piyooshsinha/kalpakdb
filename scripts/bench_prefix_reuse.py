...
def run(base: str, *, iters: int, chunk_kb: int) -> dict:
    ...
    return {
        "config": {"iters": iters, "chunk_kb": chunk_kb, "base": base},
        ...
        "note": (
            "Single-node KalpakDB-side latencies, not end-to-end TTFT. "
            "The HIT path is what an inference engine pays instead of "
            "recomputing the prefix's KV cache on a cache hit; the cross-"
...
def main() -> None:
    ...
    try:
        result = run(base, iters=iters, chunk_kb=chunk_kb)
    finally:
        if proc:
            proc.terminate()
